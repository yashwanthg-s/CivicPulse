-- Complete Cleanup and Fix for Priority Queue System
-- This script removes problematic triggers and reinstalls the system correctly
...
-- Step 2: Drop stored procedures if they exist (to reinstall cleanly)
DROP PROCEDURE IF EXISTS calculate_complaint_priority;
DROP PROCEDURE IF EXISTS update_queue_positions;
DROP PROCEDURE IF EXISTS recalculate_all_priorities;

-- Step 3: Drop view if it exists
DROP VIEW IF EXISTS complaint_priority_queue;

-- Step 4: Add priority scoring columns to complaints table (if not already present)
...
-- Step 5: Create configuration tables (if not already present)
...
-- Step 6: Clear old data from config tables and repopulate
TRUNCATE TABLE severity_config;
INSERT INTO severity_config (category, keyword, severity_score) VALUES
('infrastructure', 'gas leak', 100),
('infrastructure', 'gas leakage', 100),
...
('utilities', 'gas leakage', 100);

TRUNCATE TABLE location_sensitivity_config;
...
TRUNCATE TABLE category_sla_config;
...
-- Step 7: Create view for priority queue
...
-- Step 8: Create stored procedures (WITHOUT TRIGGERS)
DELIMITER //

CREATE PROCEDURE calculate_complaint_priority(
...
CREATE PROCEDURE recalculate_all_priorities()
BEGIN
  DECLARE v_complaint_id INT;
  DECLARE done INT DEFAULT FALSE;
...
-- Step 9: Initialize department queues
...
-- Step 10: Set department for all complaints
UPDATE complaints SET department = category WHERE department IS NULL;

-- Step 11: Recalculate all priorities
CALL recalculate_all_priorities();

-- Step 12: Update queue positions
...
-- Success message
SELECT 'Priority Queue System cleaned up and fixed successfully!' as status;
SELECT COUNT(*) as total_complaints, 
       SUM(CASE WHEN priority_score >= 200 THEN 1 ELSE 0 END) as critical,
       SUM(CASE WHEN priority_score >= 150 AND priority_score < 200 THEN 1 ELSE 0 END) as high,
       SUM(CASE WHEN priority_score >= 100 AND priority_score < 150 THEN 1 ELSE 0 END) as medium,
       SUM(CASE WHEN priority_score < 100 THEN 1 ELSE 0 END) as low
FROM complaints;
